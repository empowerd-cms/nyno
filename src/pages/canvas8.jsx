...
export default function FlowPage() {
  // Define your emoji keyword mappings here
  const keywordEmojis = {
    route: "🌐",
    db: "🗄️",
    email: "✉️",
  ...
  // ---------------------
  // Custom label renderer (prepends emoji based on keyword)
  // ---------------------
  ...
  // ---------------------
  // Style nodes dynamically (e.g., route nodes = blue)
  // ---------------------
  const renderedNodes = nodes.map((n) => {
    const labelText =
      typeof n.data.label === "string" ? n.data.label.toLowerCase() : "";
    const firstWord = labelText.split(/\s+/)[0];
    const isRoute = firstWord === "route";

    return {
      ...n,
      style: {
        ...n.style,
        background: isRoute ? "#e6f4ff" : "white",
        border: isRoute ? "1px solid #99cfff" : "1px solid #ddd",
        borderRadius: 8,
        minHeight: 80,
        padding: 10,
        textAlign: "center",
