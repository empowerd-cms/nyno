...
export default function FlowPage() {
  const initialNodes = [
    { id: "1", position: { x: 0, y: 0 }, data: { label: "Start", info: "entry point", args: ["Hello world"] }, type: "input" },
    { id: "2", position: { x: 200, y: 100 }, data: { label: "Process", info: "does something", args: [] } },
  ...
  const exportJSON = () => {
    const phpNodes = nodes.map((n) => {
      ...
      const funcName = n.data.label.replace(/\s+/g, "_");
      return {
        id: n.id,
        func: funcName,
        args: n.data.args || [],
  ...
  // ---------------------
  // Import JSON back into React Flow
  // ---------------------
  const importJSON = (e) => {
      ...
      try {
        ...
        if (data.nodes && data.edges) {
          const importedNodes = data.nodes.map((n) => ({
            id: n.id,
            position: { x: 50 * parseInt(n.id), y: 50 * parseInt(n.id) },
            data: {
              label: n.func, // function name as label
              info: "",
              args: n.args || [],
            },
          }));
